...
from data_acquisition.team_name_aliases import TEAM_NAME_ALIASES
...
def normalize_group_matches():
    ...
    for line in openfootball_cup.read_text(encoding="utf-8", errors="ignore").splitlines():
        ...
        if re.match(r"^[A-Za-z]{3} [A-Za-z]+ \d{1,2}$", stripped):
            ...
            continue
        if " v " in stripped and " @ " in stripped:
            match = re.match(
                r"^(?P<time>\d{1,2}:\d{2})\s+UTC(?P<tz>[-+0-9]+)\s+(?P<home>.*?)\s+v\s+(?P<away>.*?)\s+@\s+(?P<venue>.+)$",
                stripped,
            )
